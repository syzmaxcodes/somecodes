...
//            return Yii::$service->url->redirectByUrlKey('local_customer/account/login');
//        }
...
//    }
    ...
    public function actionIndex()
    {
        ...
        $data = $this->getBlock()->get_user_detail_info($user_info);

        return $this->render($this->action->id, ['data' => $data]);
    }
    ...
    public function actionLogin()
    {
         ...
         */
        if (!Yii::$app->user->isGuest) {
            return Yii::$service->url->redirectByUrlKey('local_customer/account');
        }
        $param = Yii::$app->request->post('editForm');
        if (!empty($param) && is_array($param)) {
            $this->getBlock()->login($param);
            if (!Yii::$app->user->isGuest) {
                return Yii::$service->customer->loginSuccessRedirect('local_customer/account');
            }
        }
    ...
    public function actionForgotpasswordsubmit()
    {
        ...
        $data = [
            'forgotPasswordUrl' => Yii::$service->url->getUrl('local_customer/account/forgotpassword'),
            'contactUrl'        => Yii::$service->url->getUrl('local_customer/contacts'),
        ];
        if (!empty($editForm)) {
            ...
            if ($identity) {
                ...
                $redirectUrl = Yii::$service->url->getUrl('local_customer/account/forgotpassword');
    ...
    public function actionResetpassword()
    {
        ...
        if (!empty($editForm)) {
            ...
            if ($resetStatus) {
                // 重置成功，跳转
                $resetSuccessUrl = Yii::$service->url->getUrl('local_customer/account/resetpasswordsuccess');
